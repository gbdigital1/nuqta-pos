...
export default function Home({ lang, onPageChange }: HomeProps) {
  ...
  const [transformStyle, setTransformStyle] = useState<string>('perspective(900px) rotateX(0deg) rotateY(0deg)');
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    // Normalize coordinates: central point is 0, bounds are -1 to +1
    const xNorm = (x / rect.width) * 2 - 1;
    const yNorm = (y / rect.height) * 2 - 1;

    // Rotate max 6 degrees
    const rotateY = (xNorm * 6).toFixed(2);
    // Invert rotateX so coordinate grids lift correctly up/down
    const rotateX = (-yNorm * 6).toFixed(2);

    setTransformStyle(`perspective(900px) rotateX(${rotateX}deg) rotateY(${rotateY}deg)`);
  };

  const handleMouseLeave = () => {
    setTransformStyle('perspective(900px) rotateX(0deg) rotateY(0deg) scale(1)');
  };

  // Fake scrolling transaction feed in terminal-style card
  const recentFeed = [
  ...
  return (
    <div className="space-y-24 pb-20">
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 md:gap-6 md:auto-rows-[200px]">
          ...
          <div className="md:row-span-2 bg-white border border-slate-200/80 rounded-[28px] shadow-sm relative overflow-hidden group hover:shadow-md transition-shadow h-[340px] md:h-auto flex flex-col justify-between">
            <div className="absolute inset-0">
              <img
                src="https://images.pexels.com/photos/37594417/pexels-photo-37594417.jpeg"
          ...
          <div className="bg-white border border-slate-200/80 rounded-[28px] shadow-sm relative overflow-hidden group hover:shadow-md transition-shadow h-[220px] md:h-auto flex flex-col justify-between">
            <div className="absolute inset-0">
              <img
                src="https://images.pexels.com/photos/37594404/pexels-photo-37594404.jpeg"
      ...
      >
        ...
        {/* Layout details: Horizontal scroll-snap on mobile, 3 grids on desktop */}
